...
def build_prompt(trace):
    """Insert a trace's JSON into the prompt template."""
    ...
    for step in steps:
        ...
        if repeat_count >= 3:
            return {
                ...
                "confidence": "high",
                "reasoning": (
                    "Detected three consecutive identical tool calls "
                    "with the same input."
                )
            }
    ...
    for step in steps:
        ...
        if repeat_count >= 3:
            return {
                ...
                "confidence": "high",
                "reasoning": (
                    "Detected three consecutive failed calls to the "
                    "same tool with identical input."
                )
            }
    ...
    for step in steps:
        ...
        if (
            ...
            return {
                ...
                "confidence": "high",
                "reasoning": (
                    "Detected a tool call with malformed or empty input."
                )
            }
...
def detect_context_overflow(trace: dict):
    ...
    if total_tokens > CONTEXT_TOKEN_THRESHOLD:
        return {
            ...
            "confidence": "high",
            "reasoning": (
                f"Total token count ({total_tokens}) exceeded the "
                f"threshold of {CONTEXT_TOKEN_THRESHOLD}."
            )
        }
...
def classify_trace(trace: dict, model: str = "gpt-4o-mini") -> dict:
    ...
    try:
        ...
        result = {
            ...
            "confidence": "low",
            "reasoning": (
                f"Failed to parse LLM response: {raw_output}"
            )
        }
